...
<div class="mx-3 container-fluid mb-5">
  ...
  <button class="btn btn-primary mb-4" data-bs-toggle="modal" data-bs-target="#addMember">
    <i class="bi bi-plus-circle"></i> Add Member
  </button>

  @if($members->count())
  <table class="table table-bordered align-middle" id="member-table" style="border-color:rgb(194, 194, 194);">
    <thead>
      <tr>
            ...
            <tr>
              ...
              <td scope="col">
                <div class="d-flex justify-content-center">
                  <button href="/member/{{ $member->id }}/edit" class="btn btn-warning d-flex align-items-center" data-bs-target="#editMember{{ $member->id }}" data-bs-toggle="modal">
                    <i class="bi bi-pencil-square @can('staff') me-1 @endcan"></i>{{ auth()->user()->role == 'Admin' ? '' : 'Edit' }}
                  </button>

                  <div class="modal fade" id="editMember{{ $member->id }}" tabindex="-1" aria-labelledby="editMemberLabel" aria-hidden="true">
                    <div class="modal-dialog">
                      <div class="modal-content">
                        ...
                        <div class="modal-body">
                          <form class="myForm" method="POST" action="/member/{{ $member->id }}">
                            @csrf
                            @method('put')
                            <div class="mb-3">
                            ...
                            <button type="submit" class="btn btn-primary submitButton">Edit Member</button>
                          </form>
                        </div>
                      </div>
                    </div>
...
<div class="modal fade" id="addMember" tabindex="-1" aria-labelledby="addMemberLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form class="myForm" method="POST" action="/member">
          @csrf
          <div class="mb-3">
            <label for="name" class="form-label">Name</label>
          ...
          <button type="submit" class="btn btn-primary submitButton">Add Member</button>
        </form>
      </div>
    </div>
  </div>
